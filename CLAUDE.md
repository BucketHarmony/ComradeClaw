...
- **generate_graphic** — Generate an SVG graphic using D3. Takes `filename` (no extension), `d3_code` (drawing code), optional `description`. The server wraps your code with jsdom + d3 boilerplate — you get `d3`, `document`, `window`, and `svg` (an 800×600 `<svg>` selection). Set `width`/`height` variables before drawing to resize. Output saved to `workspace/graphics/<filename>.svg`. If the code errors, the tool returns the error message — fix and call again (up to 3 tries). After 3 failures it returns `status: "failed"` — rewrite from scratch.
