# Skill fix: rss-infographic.md

Move this to `.claude/commands/rss-infographic.md` to apply.

Changes from previous version:
1. All three templates: `const width = 800, height = 1000;` → `width = 800; height = 1000;` (const redeclaration throws SyntaxError — wrapper already declares these)
2. STORY CARD: bullet font-size 28 → 24, bullet max length 52 → 40 chars (prevents right-edge clipping at 800px canvas)
3. Added IMPORTANT note in Step 4 header warning about the const issue
...
**Before writing d3_code:** substitute ALL CAPS_PLACEHOLDERS with real values. Truncate strings to stated limits before passing — never rely on SVG to clip. Use today's date in `APR 9, 2026` format.

**IMPORTANT:** The wrapper already declares `width` and `height`. Use **assignment** (`width = 800; height = 1000;`), NOT `const width = 800, height = 1000;` — the `const` redeclaration will throw a SyntaxError.
...
Substitute: ARTICLE_TITLE_HERE (wrap at 26 chars, max 3 lines), BULLET_1/2/3 (each max 40 chars), FEED_TITLE, TODAY_DATE.

Note: bullet font-size is 24 (not 28) to prevent right-edge clipping.
...
bullets.forEach((b, i) => {
  g.append('text').attr('x', 60).attr('y', hend+75+i*90).attr('fill', '#e63946').attr('font-size', 28).attr('font-family', 'monospace').text('\u25b8');
  g.append('text').attr('x', 100).attr('y', hend+75+i*90).attr('fill', '#ccc').attr('font-size', 24).attr('font-family', 'sans-serif').text(b.length > 40 ? b.slice(0,37)+'...' : b);
});
const bend = hend+75+3*90;
